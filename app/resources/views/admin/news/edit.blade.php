...
<div class="p-6">
    ...
    <h1 class="page-title">✏️ Edit Berita/Agenda</h1>
    ...
    <form action="{{ route('admin.news.update', $news) }}" method="POST" enctype="multipart/form-data" class="form-card">
        @csrf
        @method('PUT')
        ...
        <div class="form-grid">
            ...
            <div class="form-group full-width">
                ...
                <input type="text" name="title" value="{{ old('title', $news->title) }}" required class="form-input">
            </div>
            ...
            <div id="agendaFields" class="agenda-fields">
                <div class="form-group">
                    ...
                    <input type="date" name="event_date" value="{{ old('event_date', $news->event_date?->format('Y-m-d')) }}" class="form-input">
                </div>
                <div class="form-group">
                    <label class="form-label">🕐 Waktu Event</label>
                    <input type="time" name="event_time" value="{{ old('event_time', $news->event_time ? \Carbon\Carbon::parse($news->event_time)->format('H:i') : '') }}" class="form-input">
                </div>
                <div class="form-group">
                    <label class="form-label">📍 Lokasi Event</label>
                    <input type="text" name="event_location" value="{{ old('event_location', $news->event_location) }}" class="form-input" placeholder="Contoh: Aula Perpustakaan">
                </div>
            </div>
            ...
            <div class="form-group full-width">
                ...
                <textarea name="excerpt" rows="2" class="form-textarea" placeholder="Ringkasan singkat berita (maks 500 karakter)">{{ old('excerpt', $news->excerpt) }}</textarea>
            </div>

            <!-- Content -->
            <div class="form-group full-width">
                <label class="form-label">Konten *</label>
                <textarea name="content" rows="12" required class="form-textarea" placeholder="Tulis konten berita di sini...">{{ old('content', $news->content) }}</textarea>
            </div>
